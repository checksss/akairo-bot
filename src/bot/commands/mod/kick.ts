...
export default class BanCommand extends Command {
    ...
    public async exec(message: Message, { member, reason }: { member: GuildMember, reason: string }): Promise<Message | Message[]> {
        ...
        if (!cM.permissions.has('KICK_MEMBERS')) return message.util!.reply('I\'m not allowed to kick members.');
        if (!moderators.includes(message.author!.id)) return message.util!.reply('only moderators can kick members.');
